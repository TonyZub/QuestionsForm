...
function SetCookie(name, value){
  console.log("setting cookies to " + name + " " + value)
...
function MakeRequest(requestType, functionName, parameter){
  ...
  let requestBody = JSON.stringify(new Object({token: GetCookies()["Token"], functionName: functionName, parameter: parameter}));
  ...
  request.onload = function() {
    if (request.status != 200) {
      ...
      switch(request.status){
        case 404:
          SetInnerPageElements("", "Произошла ошибка 404 =(");
          break;
        default:
